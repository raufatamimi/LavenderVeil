...
function initLanguageSwitcher() {
  const languageLinks = document.querySelectorAll(".top-bar .dropdown-menu a[data-lang]");
  const htmlElement = document.documentElement;
  const currentLangDisplay = document.querySelector(".top-bar .dropdown-toggle i.bi-translate").nextSibling; // Get the text node next to the icon
  ...
  languageLinks.forEach(link => {
    link.addEventListener("click", function(e) {
      ...
      // Update the displayed language in the dropdown toggle
      if (currentLangDisplay && currentLangDisplay.nodeType === Node.TEXT_NODE) {
        currentLangDisplay.textContent = ` ${selectedLangText}`;
      }

      // Update selected state in dropdown
      languageLinks.forEach(l => l.querySelector(".selected-icon")?.remove()); // Remove existing checkmark
      ...
      // Re-initialize or update components that might be affected by RTL/LTR change if necessary
      // e.g., re-init swipers if their direction depends on language
      // initSwiper(); // Example: Re-init swiper if needed
    });
  });

  // Set initial state based on default lang (e.g., 'en')
  const initialLangLink = document.querySelector(".top-bar .dropdown-menu a[data-lang=\"en\"]");
  if (initialLangLink) {
      const checkIcon = document.createElement("i");
      checkIcon.className = "bi bi-check2 me-2 selected-icon";
      initialLangLink.prepend(checkIcon);
      if (currentLangDisplay && currentLangDisplay.nodeType === Node.TEXT_NODE) {
          currentLangDisplay.textContent = ` ${initialLangLink.textContent.trim()}`;
      }
  }
}

// Add language switcher initialization to existing load/ready listeners if applicable
// Or call it directly if no otherDOMContentLoaded/load logic interferes
...
function initLanguageCurrencySwitcher() {
  // Language Switcher
  const languageDropdownItems = document.querySelectorAll('.top-bar-item.dropdown .dropdown-menu .dropdown-item[data-lang]');
  const languageToggle = document.querySelector('.top-bar-item.dropdown a.dropdown-toggle i.bi-translate')?.parentElement;

  if (languageToggle && languageDropdownItems.length > 0) {
    // Load saved language or default to English
    const savedLang = localStorage.getItem('selectedLanguage') || 'en';
    const savedLangItem = document.querySelector(`.dropdown-item[data-lang="${savedLang}"]`);
    if (savedLangItem) {
        updateLanguageSelection(savedLangItem);
        // Add translation logic here if available
  ...
  function updateLanguageSelection(selectedItem) {
    if (!languageToggle) return;
    // Update toggle button text
    const selectedText = selectedItem.textContent.trim();
    // Keep the icon, update the text part
    languageToggle.innerHTML = `<i class="bi bi-translate me-2"></i> ${selectedText}`;

    // Update active state and icon in dropdown
    languageDropdownItems.forEach(i => {
        i.classList.remove('active');
        const icon = i.querySelector('i.selected-icon');
        if (icon) icon.remove();
    });
    selectedItem.classList.add('active');
    selectedItem.insertAdjacentHTML('afterbegin', '<i class="bi bi-check2 me-2 selected-icon"></i>');
  }

  // Currency Switcher
  const currencyDropdownItems = document.querySelectorAll('.top-bar-item.dropdown .dropdown-menu .dropdown-item[data-currency]');
  const currencyToggle = document.querySelector('.top-bar-item.dropdown a.dropdown-toggle i.bi-currency-dollar')?.parentElement;

  if (currencyToggle && currencyDropdownItems.length > 0) {
    // Load saved currency or default
    const savedCurrency = localStorage.getItem('selectedCurrency') || 'USD'; // Default to USD if none saved
    const savedCurrencyItem = document.querySelector(`.dropdown-item[data-currency="${savedCurrency}"]`);
    if (savedCurrencyItem) {
        updateCurrencySelection(savedCurrencyItem);
    } else {
        // If saved currency not found, default to the first one or USD
        const defaultCurrencyItem = document.querySelector('.dropdown-item[data-currency="USD"]') || currencyDropdownItems[0];
        if(defaultCurrencyItem) updateCurrencySelection(defaultCurrencyItem);
    }
    ...
    currencyDropdownItems.forEach(item => {
      item.addEventListener('click', function(e) {
        e.preventDefault();
        updateCurrencySelection(this);
        const selectedCurrency = this.getAttribute('data-currency');
        localStorage.setItem('selectedCurrency', selectedCurrency);
        // Add currency update logic here (e.g., update prices)
        console.log(`Currency changed to: ${selectedCurrency}`);
        // Optionally reload page or update prices via AJAX
      });
    });
  }

  function updateCurrencySelection(selectedItem) {
      if (!currencyToggle) return;
      // Update toggle button text
      const selectedCurrencyCode = selectedItem.getAttribute('data-currency');
      // Keep the icon, update the text part (currency code)
      currencyToggle.innerHTML = `<i class="bi bi-currency-dollar me-2"></i> ${selectedCurrencyCode}`;

      // Update active state and icon in dropdown
      currencyDropdownItems.forEach(i => {
          i.classList.remove('active');
          const icon = i.querySelector('i.selected-icon');
          if (icon) icon.remove();
      });
      selectedItem.classList.add('active');
      // Add checkmark only if it doesn't exist
      if (!selectedItem.querySelector('i.selected-icon')) {
          selectedItem.insertAdjacentHTML('afterbegin', '<i class="bi bi-check2 me-2 selected-icon"></i>');
      }
  }
  ...
  //   // Implement translation logic using data-translate-key attributes
  // }
}

// Initialize the switcher on load
document.addEventListener('DOMContentLoaded', initLanguageCurrencySwitcher);
